...
The pieces (all in `lab/manifests/frontend-canary.yaml`):
...
```bash
kubectl apply -f lab/manifests/frontend-canary.yaml
...
kubectl apply -f lab/manifests/frontend-ingressroute.yaml   # points straight at frontend (now v1, scaled 0)
```
...
```bash
kubectl delete -f lab/manifests/frontend-canary.yaml --ignore-not-found
kubectl apply -f lab/manifests/frontend-deployment.yaml -f lab/manifests/frontend-service.yaml \
